...
public abstract class AbstractOAuth2ApiBinding implements ApiBinding, InitializingBean {
	 ...
	 */
	protected AbstractOAuth2ApiBinding(AccessToken accessToken) {
	 ...
	 */
	protected AbstractOAuth2ApiBinding(AccessToken accessToken, TokenStrategy tokenStrategy) {
	...
	private RestTemplate createRestTemplate(AccessToken accessToken, OAuth2Version version, TokenStrategy tokenStrategy) {
		RestTemplate client = createRestTemplateWithCulledMessageConverters();
		ClientHttpRequestInterceptor interceptor = new OAuth2TokenParameterRequestInterceptor(accessToken, "oauth_token");
